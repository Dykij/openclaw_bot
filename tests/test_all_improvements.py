"""
Comprehensive tests for the five new improvement modules.
...
Modules under test
------------------
1. src.experience_buffer
2. src.agent_reasoning
3. src.memory_enhanced
4. src.research_enhanced
5. src.inference_optimizer
"""
...
# 1. experience_buffer
# ---------------------------------------------------------------------------
from src.experience_buffer import (
    Experience,
    ExperienceBuffer,
...
# ---------------------------------------------------------------------------
# 2. agent_reasoning  (dataclasses + ToolLearningTracker only — no LLM)
# ---------------------------------------------------------------------------
...
@pytest.fixture()
def buffer_config(tmp_dir):
    """ExperienceBufferConfig with temp storage."""
    return ExperienceBufferConfig(
        max_size=50,
        buffer_path=os.path.join(tmp_dir, "buf.jsonl"),
        seed=42,
    )
...
def buffer(buffer_config):
    return ExperienceBuffer(config=buffer_config)


# ===================================================================
# 1. experience_buffer tests
# ===================================================================


class TestExperience:
    def test_create_default(self):
        exp = Experience(prompt="p", completion="c", reward=0.9)
        assert exp.prompt == "p"
    ...
    def test_round_trip_dict(self):
        exp = Experience(prompt="hello", completion="world", reward=0.5, metadata={"k": 1})
        d = exp.to_dict()
        restored = Experience.from_dict(d)
        assert restored.prompt == exp.prompt
        assert restored.reward == exp.reward
        assert restored.metadata == {"k": 1}

    def test_from_dict_ignores_unknown_fields(self):
        d = {"prompt": "a", "completion": "b", "reward": 0.1, "unknown_field": 999}
        exp = Experience.from_dict(d)
        assert exp.prompt == "a"
        assert not hasattr(exp, "unknown_field") or True  # unknown ignored


class TestExperienceBufferConfig:
    def test_defaults(self):
        cfg = ExperienceBufferConfig()
        assert cfg.max_size == 10000
        assert cfg.min_reward_for_positive == 0.6

    def test_custom(self):
        cfg = ExperienceBufferConfig(max_size=5, seed=123)
...
class TestExperienceBufferAdd:
    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_add_single(self, buffer):
        exp = await buffer.add_experience("p1", "c1", 0.8)
        assert buffer.size == 1
        assert buffer.total_seen == 1
        assert exp.prompt == "p1"

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_add_multiple(self, buffer):
        for i in range(10):
            await buffer.add_experience(f"p{i}", f"c{i}", i / 10.0)
        assert buffer.size == 10
        assert buffer.total_seen == 10

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_reservoir_sampling_respects_max_size(self, buffer_config):
        buffer_config.max_size = 10
        buf = ExperienceBuffer(config=buffer_config)
        for i in range(100):
            await buf.add_experience(f"prompt_{i}", f"comp_{i}", i / 100.0)
        assert buf.size <= 10
        assert buf.total_seen == 100


class TestExperienceBufferSampling:
    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_sample_empty(self, buffer):
        batch = await buffer.sample_batch(5)
        assert batch == []

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_sample_returns_correct_size(self, buffer):
        for i in range(20):
            await buffer.add_experience("p", f"c{i}", i / 20.0)
        batch = await buffer.sample_batch(5)
        assert len(batch) == 5

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_sample_greedy(self, buffer):
        """Temperature=0 should return highest-priority items."""
        for i in range(10):
            await buffer.add_experience("p", f"c{i}", i / 10.0)
        batch = await buffer.sample_batch(3, temperature=0.0)
        assert len(batch) == 3

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_sample_increments_access_count(self, buffer):
        await buffer.add_experience("p", "c", 0.5)
        batch = await buffer.sample_batch(1)
        assert batch[0].access_count == 1

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_priority_favors_high_variance(self, buffer):
        """Prompts with varied rewards should be sampled more often."""
        # Prompt A: high variance
        for r in [0.1, 0.9, 0.2, 0.8]:
            await buffer.add_experience("high_var", f"c{r}", r)
        # Prompt B: low variance
        for _ in range(4):
            await buffer.add_experience("low_var", "c", 0.5)

        counts = {"high_var": 0, "low_var": 0}
        for _ in range(200):
            batch = await buffer.sample_batch(1, temperature=0.5)
            counts[batch[0].prompt] += 1
        # high-variance prompt should be sampled more often
        assert counts["high_var"] > counts["low_var"]


class TestContrastivePairs:
    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_no_pairs_for_single_experience(self, buffer):
        await buffer.add_experience("p", "c", 0.5)
        pairs = await buffer.get_contrastive_pairs("p")
        assert pairs == []

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_returns_best_worst(self, buffer):
        await buffer.add_experience("p", "good", 0.9)
        await buffer.add_experience("p", "bad", 0.1)
        pairs = await buffer.get_contrastive_pairs("p", k=1)
        assert len(pairs) == 1
        pos, neg = pairs[0]
        assert pos.reward >= neg.reward

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_respects_k_limit(self, buffer):
        for i in range(10):
            await buffer.add_experience("p", f"c{i}", i / 10.0)
        pairs = await buffer.get_contrastive_pairs("p", k=3)
        assert len(pairs) <= 3
...
class TestComputeAdvantages:
    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_empty_batch(self, buffer):
        adv = await buffer.compute_experience_advantages([])
        assert adv == []

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_single_reward(self, buffer):
        adv = await buffer.compute_experience_advantages([{"reward": 0.5}])
        assert len(adv) == 1

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_normalised(self, buffer):
        batch = [{"reward": 0.2}, {"reward": 0.8}]
        adv = await buffer.compute_experience_advantages(batch)
        assert len(adv) == 2
    ...
    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_with_buffer_history(self, buffer):
        for i in range(5):
            await buffer.add_experience("p", "c", 0.5)
        batch = [{"reward": 0.3}, {"reward": 0.7}]
        adv = await buffer.compute_experience_advantages(batch)
        assert len(adv) == 2
...
class TestCorrectionPair:
    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_no_correction_for_high_reward(self, buffer):
        result = await buffer.generate_correction_pair("p", "resp", 0.9)
        assert result is None

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_no_correction_without_better(self, buffer):
        await buffer.add_experience("p", "bad", 0.1)
        result = await buffer.generate_correction_pair("p", "bad", 0.1)
        assert result is None

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_generates_correction(self, buffer):
        await buffer.add_experience("p", "good_answer", 0.9)
        result = await buffer.generate_correction_pair("p", "bad_answer", 0.2)
        assert result is not None
        assert result["corrected_response"] == "good_answer"
...
class TestPersistence:
    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_save_load_round_trip(self, buffer_config):
        buf = ExperienceBuffer(config=buffer_config)
        for i in range(5):
            await buf.add_experience(f"p{i}", f"c{i}", i / 5.0)
        await buf.save()

        buf2 = ExperienceBuffer(config=buffer_config)
        loaded = await buf2.load()
        assert loaded == 5
        assert buf2.size == 5

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_load_missing_file(self, tmp_dir):
        cfg = ExperienceBufferConfig(buffer_path=os.path.join(tmp_dir, "nonexist.jsonl"))
        buf = ExperienceBuffer(config=cfg)
        loaded = await buf.load()
        assert loaded == 0

    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_stats(self, buffer):
...
# ===================================================================
# 2. agent_reasoning tests (dataclasses + ToolLearningTracker)
# ===================================================================
