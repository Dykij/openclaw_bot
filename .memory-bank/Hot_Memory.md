...
* Системный мозг работает на NVIDIA RTX 5060 Ti (16GB VRAM, архитектура Blackwell GB206).
* vLLM (WSL2) обслуживает AWQ-квантизированные модели через HTTP API.
* Запрещена параллельная загрузка нескольких больших LLM (во избежание Model Thrashing). Последовательная загрузка с `keep_alive=0`.
* Иерархия управления:
